...
function LocalizedFooter() {
  ...
  const t = copy.footer;
  return <footer><div className="footer-main"><div><img className="footer-logo" src="/assets/nwm-logo-header.png" alt="North Weave Mills" /><p>{t.description1}<br />{t.description2}</p></div><div><span className="eyebrow">{t.pages}</span><a href="#make">{t.capabilities}</a><a href="#process">{t.process}</a><a href="#quality">{t.quality}</a><a href="#book">{t.contactBook}</a></div><div><span className="eyebrow">{t.contact}</span><a href="https://mail.google.com/mail/?view=cm&amp;fs=1&amp;to=Info%40northweavemills.com" target="_blank" rel="noopener noreferrer">Info@northweavemills.com</a><a href="#book">{t.book}</a></div></div><div className="footer-bottom"><span>© 2026 NORTH WEAVE MILLS</span><span>{t.country}</span></div></footer>;
}
